...
ARMTF_NOBL32_EXPORTS := NEED_BL30=yes BL30=$(PWD)/$(BL30) BL33=$(PWD)/$(BL33) #CFLAGS=""

ARMTF_FLAGS := PLAT=hikey LOG_LEVEL=50 DEBUG=$(ATF_DEBUG) V=1 CRASH_REPORTING=1
...
build-opteeos:
	$(ECHO) '  BUILD   $@'
	$(Q)$(MAKE) -C my_optee_os $(optee-os-notrace-flags)

.PHONY: clean-opteeos
clean-opteeos:
	$(ECHO) '  CLEAN   $@'
	$(Q)$(MAKE) -C my_optee_os $(optee-os-notrace-flags) clean
...
#optee-test-flags := CROSS_COMPILE_HOST="$(PWD)/toolchains/$(AARCH64_GCC_DIR)/bin/aarch64-linux-gnu-" \
		    CROSS_COMPILE_TA=arm-linux-gnueabihf- \
		    TA_DEV_KIT_DIR=$(PWD)/optee_os/out/arm-plat-hisi/export-user_ta \
			CFG_CROSS_COMPILE="$(PWD)/toolchains/$(AARCH64_GCC_DIR)/bin/aarch64-linux-gnu-" \
